...
contract SBT {
    ...
    /// @param to the owner of this particular NFT, not the owner of the contract
    function mint(address to, string calldata uri) external {
        ...
        numIdentities += 1;
        allTokens[numIdentities] = numIdentities;

        // two way mapping for enumeration
        ownershipMapIndexToToken[to][balances[to]] = numIdentities;
        ownershipMapTokenToIndex[to][numIdentities] = balances[to];


        // set owner of new token
        owners[numIdentities] = to;
        // increment balances for owner
        ++balances[to];
        uriMap[numIdentities] = uri;
        emit Transfer(address(0), to, numIdentities);
        emit IdentityCreated(to, numIdentities);
    }
    ...
    function burn(uint256 tokenId) external {
        if (owners[tokenId] != msg.sender) {
            revert OnlyOwner(owners[tokenId]);
        }
        ...
        uint ownershipIndex = ownershipMapTokenToIndex[msg.sender][tokenId];
        ownershipMapTokenToIndex[msg.sender][tokenId] = 0;
        if (ownershipIndex != balances[msg.sender]) {
            uint reslottedToken = ownershipMapIndexToToken[msg.sender][balances[msg.sender]];
            ownershipMapIndexToToken[msg.sender][ownershipIndex] = reslottedToken;
            ownershipMapIndexToToken[msg.sender][balances[msg.sender]] = 0;
            ownershipMapTokenToIndex[msg.sender][reslottedToken] = ownershipIndex;
        } else {
            ownershipMapIndexToToken[msg.sender][ownershipIndex] = 0;
        }
